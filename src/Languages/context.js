...
export function LanguageProvider({ defaultLanguage, children }) {
  ...
  const provider = {
    ...
    setLanguage(language) { if (language in languages) setLanguage(language) }, // A function to update the language
    translate(term) { return (term in resources ? resources[term] : term) } // A simple translation function for anyone who just needs a value translated
  };
